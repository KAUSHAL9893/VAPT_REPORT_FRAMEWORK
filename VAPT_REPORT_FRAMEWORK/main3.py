...
class VAPTReportGenerator:
    ...
    def generate_report(self, report_data, template_name, output_format='pdf', output_file=None, temp_dir=None):
        ...
        if output_format == 'html':
            ...
            try:
                # Try using WeasyPrint first (better styling support)
                html = HTML(filename=html_file)
                # Get template directory for CSS resources
                css_files = [
                    CSS(filename=os.path.join(self.template_dir, 'pdf_styles.css')) 
                    if os.path.exists(os.path.join(self.template_dir, 'pdf_styles.css')) 
                    else None
                ]
                css_files = [css for css in css_files if css]
                
                html.write_pdf(output_file, stylesheets=css_files)
                print(f"PDF generated using WeasyPrint: {output_file}")
            except Exception as e:
                print(f"Error with WeasyPrint: {e}")
                print("Falling back to wkhtmltopdf...")
                try:
                    ...
                    print(f"PDF generated using wkhtmltopdf: {output_file}")
